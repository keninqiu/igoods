...
class Membership_Activity_Model_Activity extends Membership_Base_Model_Base {
	...
	public function buildActivitySelectQuery(array $queryParams) { // $whereArray, $requestedUserId, $type, $limit, $offsetId = null) {
		...
		$dataOr = '';
		if(isset($queryParams['dataLike'])) {
			foreach ($queryParams['dataLike'] as $str) {
				$dataOr .= "a.data LIKE '%".$str."%' AND ";
			}
			if (!empty($dataOr)) {
				$dataOr = ' AND ('.substr($dataOr, 0, -4).')';
			}
		}
	...
	public function getActivity(array $params) {
		...
		$dataLike = isset($params['dataLike']) ? $params['dataLike'] : null;
	...
	private function prepareActivityRelatedData($activities, $currentUserId) {
		...
		foreach ($activities as $key => &$activity) {
			...
			if (strpos($activity['type'], 'shared_') !== false) {
				...
				if(isset($fetchedData['users'][$activity['user_id']])) {
					$activity['author'] = $fetchedData['users'][$activity['user_id']];
				}
				$activity['sharedActivity'] = null;
				if(isset($fetchedData['related_activities'][$activity['foreign_id']])) {
	...
	public function updateFewActivitiesStatus($activityIdArr, $status) {
		if(count($activityIdArr)) {
			$currentDateTime = $this->getCurrentDateInUTC();
			$sqlParams = array();
			$sqlParams[] = $status;
			$sqlParams[] = $currentDateTime;

			$sqlStrPrm = array_fill(0, count($activityIdArr), '%d');

			foreach($activityIdArr as $oneId) {
				$sqlParams[] = $oneId;
			}

			$query = $this->preparePrefix("UPDATE {prefix}activity SET `status` = '%s', `updated_at` = '%s' 
				WHERE `id` IN(" . implode(',', $sqlStrPrm) . ");");

			$query = $this->db->prepare($query, $sqlParams);
			$this->db->query($query);
		}
	}
	...
	public function getActivityIdsByUserId($userId) {
		$ids = array();

		$userQueryIds = $this->preparePrefix("
			SELECT id AS activity_id
			FROM {prefix}activity 
			WHERE user_id = %d
		");
		$userQueryIds = $this->db->prepare($userQueryIds, array($userId));

		$tmpIds = $this->db->get_results($userQueryIds, ARRAY_A);
		if(count($tmpIds)) {
			foreach($tmpIds as $oneItem) {
				$ids[] = $oneItem['activity_id'];
			}
		}
