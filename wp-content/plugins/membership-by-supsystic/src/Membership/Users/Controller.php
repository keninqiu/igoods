...
class Membership_Users_Controller extends Membership_Base_Controller {
	...
	public function saveAboutFields(Rsc_Http_Parameters $parameters) {
		...
		if (in_array($fieldName, array('first_name', 'last_name', 'nickname'))) {
			wp_update_user(
				array(
					'ID' => $userId,
					$fieldName => $fieldData
...
//		$extraWhere = "AND (um.meta_value REGEXP '.*\"show-in-members-directory\";s:[0-9]+:\"yes\".*' OR um.meta_value IS NULL)";
    ...
    public function bulkUpdate(Rsc_Http_Parameters $parameters) {
		...
		switch ($action) {
			case 'change-status':
				...
				$newStatus = intval($value);
				if (in_array($newStatus, array_keys($statuses))) {
					$mailModule = $this->getModule('Mail');

					foreach ($users as $userId) {
						$user = $profileModel->getUserById(intval($userId));

						if ($profileModel->setUserStatus($userId, $newStatus)) {
							$oldStatus = is_null($user['user_status']) ? Membership_Users_Model_Fields::STATUS_PENDING_REVIEW : intval($user['user_status']);
							
							if (Membership_Users_Model_Fields::STATUS_PENDING_REVIEW === $oldStatus) {
								switch ($newStatus) {
									case Membership_Users_Model_Fields::STATUS_ACTIVE:
									$mailModule->sendAccountApprovedEmail($user);
									break;
	...
	public function wpUserRemoveHandler($userId, $reassign = null) {
		$userId = (int)$userId;
		$profileModel = $this->getModel('Profile');
		$activityModel = $this->getModel('activity', 'activity');
		$activityIds = $activityModel->getActivityIdsByUserId($userId);

		// update activity status to delete wp_supsystic_membership_activity
		$activityModel->updateFewActivitiesStatus($activityIds, 'deleted');

		// update user status in
		$profileModel->setUserStatus($userId, Membership_Users_Model_Fields::STATUS_DELETED);
	}
}