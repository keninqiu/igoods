...
class Membership_Membership_Controller extends Membership_Base_Controller
{
	public function indexAction(Rsc_Http_Request $request)
	{
		...
		$isUltimateMemberExists = in_array('ultimate-member/ultimate-member.php', array_keys(get_plugins()));
		...
		if(is_array($editableRoles)) {
			foreach($editableRoles as $roleKey => $oneRole) {
				if(!empty($oneRole['name'])) {
					$wpRoleList[$roleKey] = $oneRole['name'];
				}
			}
		}
