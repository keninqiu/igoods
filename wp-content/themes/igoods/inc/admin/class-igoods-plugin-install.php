...
if ( ! class_exists( 'Igoods_Plugin_Install' ) ) :
	/**
	 * The Igoods plugin install class
	 */
	class Igoods_Plugin_Install {
		...
		public function plugin_install_scripts( $hook_suffix ) {
			...
			wp_enqueue_script( 'igoods-plugin-install', get_template_directory_uri() . '/assets/js/admin/plugin-install' . $suffix . '.js', array( 'jquery', 'updates' ), $igoods_version, 'all' );

			wp_enqueue_style( 'igoods-plugin-install', get_template_directory_uri() . '/assets/css/admin/plugin-install.css', array(), $igoods_version, 'all' );
		}
			...
			if ( current_user_can( 'install_plugins' ) && current_user_can( 'activate_plugins' ) ) {
				if ( is_plugin_active( $plugin_slug . '/' . $plugin_file ) ) {
					...
					$button = array(
						...
						'url'     => '#',
						'classes' => array( 'igoods-button', 'disabled' ),
					);
					...
					$button = array(
						...
						'url'     => $url,
						'classes' => array( 'igoods-button', 'activate-now' ),
					);
					...
					$button = array(
						...
						'url'     => $url,
						'classes' => array( 'igoods-button', 'sf-install-now', 'install-now', 'install-' . $plugin_slug ),
					);
...
return new Igoods_Plugin_Install();
